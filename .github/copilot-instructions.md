...
- **Binaries & helpers:** shell wrapper `bin/cljx` is the preferred entrypoint for common developer flows (build, repl, evaluate source).
...
- The runtime uses shared ownership + interior mutability heavily: `Rc<Value>`, `RefCell<HashMap<...>>` for `Context`.
...
- Entry & CLI: [src/main.rs](src/main.rs)
- Shell wrapper & flows: [bin/cljx](bin/cljx)
- Runtime model: [crates/cljx_value/src/lib.rs](crates/cljx_value/src/lib.rs)
- Root workspace config: [Cargo.toml](Cargo.toml)
- Usage examples: [README.md](README.md) and [samples/old.cljx](samples/old.cljx)

## Integration points & external tooling
- The `bin/cljx` script assumes `pkgx` is available on `PATH`; `pkgx` then provides `yq` and `bb` which the script uses to process embedded YAML flows.
...
- For CLI behavior, prefer modifying `src/main.rs` and `bin/cljx` flows together to keep script and binary in sync.
