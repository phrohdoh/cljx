...
impl Value {
    ...
    pub fn list_from(items: Vec<RcValue>) -> Self {
        Self::List(List::new(items), Meta::new_empty_rc())
    }
    ...
    pub fn vector_from(items: Vec<RcValue>) -> Self {
        Self::Vector(Vector::new(items), Meta::new_empty_rc())
    }
...
impl Value {
    pub fn with_meta(self, meta: RcMeta) -> Self {
        match self {
            ...
            Value::Boolean(boolean, _)   => Value::Boolean(boolean, meta),
            Value::Integer(integer, _)   => Value::Integer(integer, meta),
            Value::Float(float, _)       => Value::Float(float, meta),
            Value::String(string, _)     => Value::String(string, meta),
            Value::Symbol(symbol, _)     => Value::Symbol(symbol, meta),
            Value::Keyword(keyword, _)   => Value::Keyword(keyword, meta),
            Value::List(list, _)         => Value::List(list, meta),
            Value::Vector(vector, _)     => Value::Vector(vector, meta),
            Value::Set(set, _)           => Value::Set(set, meta),
            Value::Map(map, _)           => Value::Map(map, meta),
            Value::Var(var, _)           => Value::Var(var, meta),
            Value::Function(function, _) => Value::Function(function, meta),
            Value::Handle(handle, _)     => Value::Handle(handle, meta),
        }
    }
    ...
    pub fn new_list(elements: Vec<RcValue>) -> Self {
        Self::List(List::new(elements), Meta::default().into_meta_rc())
    }

    pub fn new_list_rc(elements: Vec<RcValue>) -> RcValue {
        Rc::new(Self::List(List::new(elements), Meta::default().into_meta_rc()))
    }
}
...
impl Value {
    ...
    pub fn new_vector(elements: Vec<RcValue>) -> Self {
        Self::Vector(Vector::new(elements), Meta::default().into_meta_rc())
    }

    pub fn new_vector_rc(elements: Vec<RcValue>) -> RcValue {
        Rc::new(Self::Vector(Vector::new(elements), Meta::default().into_meta_rc()))
    }
}
