//! A [`Keyword`] is a [`Symbol`] that `eval`uates to itself.
//! It may be either unqualified ([`Keyword::Unqualified`]) or qualified ([`Keyword::Qualified`]).
...
impl Keyword {
    ...
    pub fn namespace_or_panic(&self) -> &str {
        self.namespace().expect(&format!("attempted to get namespace of unqualified keyword: {}", self))
    }
}
