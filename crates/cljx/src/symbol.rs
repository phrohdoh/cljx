//! A [`Symbol`] is an identifier that `eval`uates to a `Value`.
//! It may be either unqualified ([`Symbol::Unqualified`]) or qualified ([`Symbol::Qualified`]).
...
impl Symbol {
    ...
    pub fn namespace_or_panic(&self) -> &str {
        self.namespace().expect(&format!("attempted to get namespace of unqualified symbol: {}", self))
    }
}
