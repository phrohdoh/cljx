...
impl From<Vec<RcValue>> for List {
...
impl List {
    ...
    pub fn new_value_rc(elements: Vec<RcValue>) -> RcValue {
        Value::list_rc(Self::from(elements))
    }
...
mod tests {
    ...
    fn inequality_with_different_elements() {
        let list1 = List::from(vec![Value::integer_rc(1)]);
        let list2 = List::from(vec![Value::integer_rc(2)]);
    ...
    #[test]
    fn into_value() {
        let list = List::from(vec![Value::integer_rc(42)]);
    ...
    #[test]
    fn into_value_rc() {
        let list = List::from(vec![Value::integer_rc(99)]);
